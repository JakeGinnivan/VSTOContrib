...
using System.Linq;
using Outlook.Utility.Properties;
...
namespace Outlook.Utility.Services
{
    ...
    public class GenericSynchronisationService<TType, TKey> where TType : class
    {
        ...
        private SynchronisationResults TwoWaySync()
        {
            ...
            var remoteDeleted = _remoteProvider.GetDeletedEntries(_settings.LastSync);
            var localDeleted = _localProvider.GetDeletedEntries(_settings.LastSync);
        ...
        private SynchronisationResults DownloadChanges()
        {
            List<TType> saveLocal;
            List<TType> saveRemote;
            var localDeleted = _remoteProvider.GetDeletedEntries(_settings.LastSync);
