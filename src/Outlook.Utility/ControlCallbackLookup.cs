...
namespace Outlook.Utility
{
    internal class ControlCallbackLookup
    {
        ...
        public ControlCallbackLookup(Dictionary<string, Dictionary<string, Expression<Action>>> controlCallbackLookup)
        {
            ...
            foreach (var control in controlCallbackLookup.Where(c => c.Key != RibbonFactory.RibbonFactory.CommonCallbacks))
            {
                ...
                foreach (var controlCallbacks in control.Value.Concat(controlCallbackLookup[RibbonFactory.RibbonFactory.CommonCallbacks]))
                {
