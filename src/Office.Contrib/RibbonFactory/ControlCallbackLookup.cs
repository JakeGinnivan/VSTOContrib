...
{
    internal class ControlCallbackLookup<TRibbonType> where TRibbonType : struct
    {
        /// <summary>
        ...
        public ControlCallbackLookup(IDictionary<string, Dictionary<string, Expression<Action>>> controlCallbackLookup)
        {
            ...
            foreach (var control in controlCallbackLookup.Where(c => c.Key != RibbonFactory.RibbonFactory<TRibbonType>.CommonCallbacks))
            {
                ...
                foreach (var controlCallbacks in control.Value.Concat(
                    controlCallbackLookup[RibbonFactory.RibbonFactory<TRibbonType>.CommonCallbacks]))
                {
